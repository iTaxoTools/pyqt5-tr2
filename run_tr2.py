...
def model_comparison(treefile, assocfile, N="ALL"):
	...
	L = len(triple_count)
	nullscore = ppmodel.calculate(list(triple_count.values()), ["NULL"]*L)
	...
	for i in range(N):
		...
		mixscores.append(ppmodel.calculate(list(triple_count.values()), categorize_triples(triple_count, tip_group)))
