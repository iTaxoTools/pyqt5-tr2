...
#output a list of three possible topology of triplet
def all_topology(members):
	ms = list(members)
	triples = []
	for i in range(3):
		triples.append(frozenset([frozenset([ms[i]]), frozenset(ms[:i]+ms[(i+1):])]))
...
def count_triples(f, num_tree="ALL", keep_topology=False):
	...
	if not keep_topology:
		for k, v in list(triple_count.items()):	#replace dictionary with list of counts
			triple_count[k] = list(triple_count[k].values())
			if len(triple_count[k]) < 3:	#if length of count is smaller than 3, append 0's
				triple_count[k].extend([0]*(3-len(triple_count[k])%3))
	else:
		for k, v in list(triple_count.items()):
			if len(triple_count[k]) < 3:	#if length of count is smaller...
				alltopo = all_topology(k)
				#print (alltopo,triple_count[k])
				for t in alltopo:
					if not t in triple_count[k]:
						triple_count[k][t] = 0
