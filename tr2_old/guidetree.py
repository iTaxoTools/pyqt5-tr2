...
class GuideTree:
	def __init__(self, tree, sep="<>"):
		#tree = TreeNode.tree_from_newick(tree)
		self.root = tree
	...
	def list_all_triplets(self):
		if len(self.root.name) >= 3:
			return set([frozenset(x) for x in itertools.combinations(self.root.name, 3)])
		else:
			return None

	def list_tip_names(self):
		if self.root.name:
			return set(self.root.name)
		else:
			return None
